...
export default function DailyUsageDashboard({ dailyUsageRecords }: DailyUsageDashboardProps) {
    ...
    return (
        <Card className="w-full max-w-4xl mx-auto bg-card/90 backdrop-blur-sm">
            <CardHeader>
                ...
                {dailyUsageRecords.length > 0 ? (
                    <Accordion type="single" collapsible className="w-full space-y-4">
                        {dailyUsageRecords.map(record => (
                            <AccordionItem key={record.id} value={record.id} className="bg-background/70 backdrop-blur-sm border-none rounded-lg">
